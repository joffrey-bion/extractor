...
public class Main {
	...
	private static void downloadBigImg(LinkedList<Pokemon> list, String destFolder) {
		for (Pokemon p : list) {
			...
			download("http://img.pokemondb.net/artwork/" + name.toLowerCase() + ".jpg", destFolder + "/" + p.num
					+ ".jpg");
		}
	}

	private static void writeToSql(LinkedList<Pokemon> list) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(PATH_SQL))) {
			for (Pokemon p : list) {
	...
	private static void writeToCsv(LinkedList<Pokemon> list) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(PATH_CSV))) {
